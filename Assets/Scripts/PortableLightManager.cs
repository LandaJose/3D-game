...
public class PortableLightManager : MonoBehaviour
{
    ...
    public LanternPickUp lanternAudio;
    public FlashLightPickUp flashLightAudio;
    public GeneratorTurnOn generatorAudio;
    void Start()
    {
        lanternAudio = lantern.GetComponent<LanternPickUp>();
        flashLightAudio = flashlight.GetComponent<FlashLightPickUp>();
        generatorAudio = generator.GetComponent<GeneratorTurnOn>();
        genOn = false;
    ...
    void PickUp()
    {
        ...
        if (Physics.Raycast(ray, out hit, 3.0f))
        {
            ...
            {
                flashLightAudio.PlayFlashAudio();
            ...
            else if (selection.gameObject.Equals(generator))
            {
                for (int i = 0; i < inventory.slots.Length; i++)
                {
                    ...
                    {
                        generatorAudio.PlayGeneratorAudio();
                        genOn = true;
                        activeLight = "house";
                        TurnOnLight();
                        EnableOilCans(false);
